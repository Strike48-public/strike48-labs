...
:::tip
If your kubeconfig uses token-based auth (e.g., EKS, GKE), credentials may expire. Re-run the provider's CLI to refresh:
...
:::caution
Avoid disabling TLS verification in production. If you're using a private CA, add it to your system trust store instead.
:::
