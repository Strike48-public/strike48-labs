...
:::caution
Trace-level kube-rs logging includes full response bodies, which can be very large for list operations. Use this only for targeted debugging.
:::
...
:::tip
Strip any sensitive data (bearer tokens, cluster addresses, secret values) from log files before sharing.
:::
