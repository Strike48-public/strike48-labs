...
:::note
KubeStudio displays pod events in the pod detail view. Check there first before switching to the terminal.
:::
...
:::tip
If an HPA is managing the deployment, manual scaling will be overridden on the next reconciliation cycle. Adjust the HPA's `minReplicas`/`maxReplicas` instead.
:::
