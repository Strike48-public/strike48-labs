...
Pick is a headless penetration testing agent that runs as a StrikeHub connector or standalone binary.
...
Run Pick directly, connecting to a Prospector Studio backend:

```bash
./target/release/pentest-agent
```
