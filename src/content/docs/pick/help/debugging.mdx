...
### Headless mode (standalone)

Logs go directly to stderr:
...
Or run Pick standalone to debug in isolation, then switch back to IPC mode once resolved.
...
:::tip
Strip any sensitive data (target IPs, network scan results, credentials) from log files before sharing.
:::
