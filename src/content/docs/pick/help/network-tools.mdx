...
:::tip
If scanning takes a long time, the target may have a firewall that drops packets silently (no RST response). Reduce the port range or increase the timeout.
:::
...
:::note
WiFi scanning on headless Linux servers without a wireless adapter will always return empty. This tool is designed for devices with WiFi hardware.
:::
