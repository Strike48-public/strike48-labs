...
:::caution
Running the entire application as root is acceptable for development but not recommended for production deployments. Use Linux capabilities or per-tool escalation instead.
:::
...
:::note
`setcap` only works on the final binary, not via `cargo run`. Build a release binary first with `cargo build --release -p pick-connector`.
:::
