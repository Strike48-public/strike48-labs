...
Pick is a droppable penetration testing toolkit best run hosted within Prospector Studio or the StrikeHub desktop app. It's designed as a single binary that can be deployed on any system without requiring a GUI, windowing system, or desktop environment. Pick provides network scanning, device discovery, traffic capture, and remote command execution — all controllable locally via a workspace UI or remotely via the Prospector Studio API.
